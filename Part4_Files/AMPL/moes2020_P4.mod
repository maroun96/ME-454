...
var a 						>= 0.000001; #factor for fitting function for carnot factor 
var b 						>= 0.000001;#factor for fitting function for carnot factor 
var c 						>= 0.000001; #factor for fitting function for carnot factor 
...
log((T_cond[12]-T_medium_out)/(T_cond[12]-T_medium_in))*DTlnCond = ((T_cond[12]-T_medium_out) - (T_cond[12]-T_medium_in));	
...
Q_cond[12] = U_water_ref*DTlnCond*Cond_area;

subject to Comp1cost: 
#calculates the cost for comp1 for extreme period 
comp_cost = 10^(k1 + k2*log10(W_comp2[12])+k3*(log10(W_comp2[12]))^2)*(index/ref_index)*f_BM*0.98;
  	
 #subject to HEX2_cost: #calculates the cost for HEX2 for extreme period 
 subject to Condenser_cost:
 Cond_cost = 10^(k1_HEX + k2_HEX*log10(Cond_area)+k3_HEX*(log10(Cond_area))^2)*(index/ref_index)*f_BM_HEX*0.98;
